...
function LeaveMgmtContent() {
  ...
  useEffect(() => {
    fastGet('/attendance/teachers')
      ...
      .catch((err) => console.error('Failed to load staff list for leave:', err));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/teacher-portal/leave', {
        teacherId: selectedStaffId || undefined,
        startDate,
        endDate,
        leaveType,
        reason,
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 bg-white dark:bg-slate-800">
          {staffList.length > 0 && (
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Staff Member / Faculty</label>
              <select
              ...
              >
                {staffList.map((s: any) => (
                  <option key={s.id} value={s.id} className="dark:bg-slate-800">
                    {s.name} ({s.subject || s.designation || 'Faculty'})
                  </option>
                ))}
              </select>
            </div>
