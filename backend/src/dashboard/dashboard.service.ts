...
export class DashboardService {
  ...
  async getDashboardSummary() {
    ...
    console.log(`[PERF] dashboard.summary.start tenant=${tenantId}`);
    ...
    if (_cached && _cached.expiresAt > Date.now()) {
      const cacheElapsed = Math.round((performance.now() - _t0) * 10) / 10;
      console.log(`[PERF] dashboard.summary.total [CACHE HIT] tenant=${tenantId} duration=${cacheElapsed}ms`);
      return _cached.data;
    }
    ...
    const [
      ...
      `.catch((err) => {
        console.error('[PERF ERROR] dashboard.metrics query failed:', err?.message || err);
        return [{}];
      }),
      ...
      `.catch((err) => {
        console.error('[PERF ERROR] dashboard.academicAverage query failed:', err?.message || err);
        return [{ avgScore: 0 }];
      }),
    ...
    const dbElapsed = Math.round((performance.now() - _tDb) * 10) / 10;
    console.log(`[PERF] dashboard.academicAverage [DB] tenant=${tenantId}`);
    console.log(`[PERF] dashboard.attendanceRate [DB] tenant=${tenantId}`);
    console.log(`[PERF] dashboard.monthlyMetrics [DB] tenant=${tenantId}`);
    console.log(`[PERF] dashboard.otherQueries [DB] tenant=${tenantId} duration=${dbElapsed}ms`);
