...
export class TimetableService {
  ...
  async createClassSection(dto: any) {
    ...
    if (classSection) {
      ...
      const createAssigns = Object.entries(dto.subjectTeacherMap || {}).flatMap(([subjectId, teacherIds]) => {
        ...
        return ids
          .filter((tId: any) => tId && String(tId).trim() !== '')
          .map((teacherId: string, index: number) => {
  ...
  async getWorkloadDashboardData() {
    ...
    for (const t of teachersList) {
      ...
      totalLoad += Math.min(100, Math.round((totalPeriods / 25) * 100));
    }
    ...
    const mappedTeachers = teachersList.map(t => {
      ...
      const loadPercent = Math.min(100, Math.round((totalPeriods / 25) * 100));
    ...
    const mappedClassSections = classSections.map(cs => {
      ...
      const staffedSubjectIds = new Set(
        cs.teacherAssigns.filter(a => a.teacherId && a.subjectId).map(a => a.subjectId)
      );
      const staffedCount = subjectCount > 0
        ? cs.classSubjects.filter(csSub => staffedSubjectIds.has(csSub.subjectId)).length
        : staffedSubjectIds.size;
      const loadPercent = subjectCount > 0
        ? Math.min(100, Math.round((staffedCount / subjectCount) * 100))
        : (staffedSubjectIds.size > 0 ? 100 : 0);
  ...
  async getWorkloadSummary(academicYearId: string) {
    ...
    // Calculate avgLoadPercent (aggregate based on total assigned periods vs 25 standard weekly load)
